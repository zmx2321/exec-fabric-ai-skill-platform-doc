...
<div class="brand-page-shell">
  <section class="brand-hero">
    <div>
      ...
      <p class="brand-lead">
        璇络(ExecFabric)的核心能力不是零散功能点，而是一套围绕
  ...
  <section class="brand-grid brand-grid--two">
    ...
    <article class="brand-card">
      ...
      <ul class="brand-list">
        ...
        <li>对话执行支持按平台默认或租户范围维护上游模型接入配置</li>
        <li>平台超管和租户管理员现在都可以维护自己的大模型列表</li>
        <li>超管平台模型要先授权到租户，再下发到具体用户；用户在公告中心确认后，模型才会进入自己的对话执行可选范围</li>
        <li>平台治理侧支持按单租户或批量维护租户大模型地址与 Token</li>
        <li>批量维护既可覆盖所选租户，也可只补本地空缺租户</li>
        <li>敏感 Token 的完整值只对配置方可见，另一侧只显示遮罩</li>
        <li>个人免费版普通用户不需要进入“大模型列表”，也可以在对话执行里切换自己已授权的模型</li>
        <li>会话模型会参与 Skill 门槛校验，不再只按默认模型盲目执行</li>
      </ul>
    </article>
  </section>
  ...
  <section class="brand-card">
    ...
    <ul class="brand-list">
      ...
      <li>第一种非脚本样板是平台治理控制台的 HTTP 能力接入，用来验证资源模型、执行器和审计链路</li>
      <li>租户侧已先提供 HTTP 资源只读查看页，用来展示当前账号被授权资源的请求摘要、鉴权方式和写边界，但不开放租户自助配置与试跑</li>
      <li>HTTP 资源当前仍只按“固定接口 + 显式鉴权 + 写操作白名单”推进，配置权和凭据治理继续只放在平台治理控制台</li>
  ...
  <section class="brand-grid brand-grid--two">
    <article class="brand-card">
      ...
      <h2>低客服闭环已经先落到哪</h2>
      <ul class="brand-list">
        <li>平台治理控制台订单中心已支持建单、确认支付、执行开通、续费单预填、续费提醒扫描、到期停开扫描</li>
        <li>璇络个人免费版前台已支持本地脚本位轻量自助增购，确认后会直接回填当前账号额度</li>
        <li>璇络个人免费版也已开放团队版月租 / 年租升级入口，会把用户引导到标准 SaaS 正式租户注册路径</li>
    ...
    <article class="brand-card">
      ...
      <h2>当前还没对外承诺什么</h2>
      <ul class="brand-list">
        <li>还没有开放成完整通用的终端用户支付中心；当前成立的是璇络个人免费版本地脚本位轻量自助增购，以及团队版升级注册入口</li>
  ...
  <section class="brand-grid brand-grid--two">
    <article class="brand-card">
      ...
      <ul class="brand-list">
        ...
        <li>租户安装申请与确认安装</li>
        <li>版本管理、回滚和分发日志</li>
        <li>平台模板分发是租户级安装；受控演示能力分发是用户级授权，当前公开示例是 POI 测试脚本包，不单独作为一级产品模块宣传</li>
    ...
    <article class="brand-card">
      ...
      <ul class="brand-list">
        <li>高风险动作继续保留人工确认</li>
        <li>平台治理域可维护模型注册表与可靠性评级（经济级 / 标准级 / 高可靠级）</li>
        <li>每个 Skill 可配置最低模型可靠性门槛（所有模型可用 / 标准级及以上 / 仅限高可靠）</li>
        <li>当模型未评级、已停用或等级不足时，运行时会拒绝执行并提示切换更高可靠模型</li>
  ...
  <section class="brand-grid brand-grid--two">
    <article class="brand-card">
      ...
      <ul class="brand-list">
        ...
        <li>个人免费入口当前已经形成注册结果页、“我的技能库 / 脚本管理（上传脚本 / 脚本列表） / 对话执行 / 执行历史 / 个人空间 / 公告中心 / 社区广场 / 升级正式版”这组有效菜单、HTTP 只读台账和本地脚本位轻量增购等连续使用入口</li>
      </ul>
    </article>
    <article class="brand-card">
  ...
  <section class="brand-card">
    ...
    <p>当前公开能力仍以 Python 脚本接入为主，但平台已经明确预留下面这些扩展方向。这里表达的是演进边界，不是“已经全部标准商用”。其中 HTTP API 样板已经先在平台治理控制台落地，用来验证接入路径。</p>
