...
<section class="brand-hero">
  <div>
    ...
    <p class="brand-lead">
      ...
      它的目标不是讲概念，而是把<strong>定位、边界、隔离、热更新、CLI、本地执行和当前阶段</strong>一次说清楚。
    </p>
    <div class="brand-chip-list">
      <span class="brand-chip">定位</span>
      <span class="brand-chip">隔离</span>
  ...
  <section class="brand-card">
    ...
    <div class="brand-grid brand-grid--two">
      <article class="brand-card brand-card--nested">
        <h3>它是不是一个普通聊天系统</h3>
        <p>不是。它是一个让 AI 在规则内调用和执行真实能力的平台，当前最成熟的执行资产是 Python 脚本；同时 Web 端上传注册已经支持 Python / Shell / Bat(Cmd) / Node.js。</p>
      </article>
      <article class="brand-card brand-card--nested">
        <h3>它适合哪些人</h3>
        <p>适合已经有脚本、自动化任务、数据处理需求，或需要把这些能力变成统一入口、统一交付和统一治理的人。</p>
      </article>
      <article class="brand-card brand-card--nested">
        <h3>免费版是不是企业后台裁剪版</h3>
  ...
  <section class="brand-card">
    ...
    <div class="brand-grid brand-grid--two">
      ...
      <article class="brand-card brand-card--nested">
        ...
        <p>不会。当前推荐范围受租户与权限共同约束，不是“模型想到什么就推什么”。</p>
      </article>
      <article class="brand-card brand-card--nested">
        <h3>对话执行的大模型地址和 Token 是统一一套吗</h3>
        <p>不是只能统一一套。平台可以维护默认配置，正式租户也可以维护自己的本地配置；如果租户本地配置不完整，运行时会继续回退平台默认配置。现在公开口径还要再补一层：平台超管维护平台模型库，租户管理员维护本租户的大模型列表；超管平台模型要先授权到租户，再下发到具体用户；用户在公告中心确认后，模型才会进入自己的对话执行可选范围。个人免费版普通用户不需要单独的大模型列表页，也可以直接在对话执行里切换自己已授权的模型。为了避免敏感信息横向暴露，完整 Token 只对配置方可见，另一侧只显示遮罩。</p>
      </article>
      <article class="brand-card brand-card--nested">
  ...
  <section class="brand-card">
    ...
    <div class="brand-grid brand-grid--two">
      ...
      <article class="brand-card brand-card--nested">
        ...
        <p>网页端上传注册现在已经支持 Python / Shell / Bat(Cmd) / Node.js 文本脚本；页面会按当前云端环境自动判断是否开放 <code>bat / cmd</code>，非 Windows 云端会直接限制。CLI 远程注册当前也已支持 Python / Shell / Bat(Cmd) / Node.js；如果是本机路径登记，CLI / Local Agent 已可登记 Python / Shell / Bat / Node。需要注意的是，<code>bat / cmd</code> 在线执行依赖 Windows 运行时。</p>
      </article>
    </div>
  </section>
  ...
  <section class="brand-card">
    ...
    <div class="brand-grid brand-grid--two">
      ...
      <article class="brand-card brand-card--nested">
        <h3>体验页适合帮我判断什么</h3>
        <p>它适合先感受执行链路、理解平台定位，并判断自己更适合继续走免费线、本地脚本位、标准团队版还是企业咨询。当前体验页左侧是 Python `task_brief_demo.py`，右侧是 Shell `clean_log_demo.sh`，对应脚本来自正式的免费线体验脚本目录。</p>
      </article>
      <article class="brand-card brand-card--nested">
        <h3>注册成功后会去哪里</h3>
